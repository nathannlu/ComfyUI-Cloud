...
from .progress import progress_dict, progress_update

HASH_CHUNK_SIZE = 4096

def _update(hashers, data: Union[bytes, IO[bytes]], filename: str = None):
    if isinstance(data, bytes):
        ...
        while 1:
            ...
            #print(f'\rProgress: {progress_dict}', end='', flush=True)
            progress_update(filename=filename, value=total_size - processed_size, max=total_size)
...
def get_upload_hashes(data: Union[bytes, IO[bytes]], filename: str = None) -> Tuple[UploadHashes, str]:
    md5 = hashlib.md5()
    sha256 = hashlib.sha256()
    _update([md5, sha256], data, filename)
