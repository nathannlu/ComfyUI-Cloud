...
async def upload_dependencies(request):
    ...
    vol = modal.Volume.lookup("your_item_name")
    ...
    custom_nodes_dep = json_data["dependencies"]["custom_nodes"]
    models_dep = json_data["dependencies"]["models"]

    try:
        with vol.batch_upload(force=True) as batch:
            for typ, models_array in models_dep.items():
                for item in models_array:
                    batch.put_file(f"../../models/{typ}/{item['name']}", f"/vol/{workflow_id}/comfyui/models/{typ}/{item['name']}")
            print("Uploaded models")

            for git_url, custom_node_obj in custom_nodes_dep.items():
                batch.put_directory(f"../../custom_nodes/{custom_node_obj['name']}", f"/vol/{workflow_id}/comfyui/custom_nodes/{custom_node_obj['name']}")
            print("Uploaded custom nodes")
